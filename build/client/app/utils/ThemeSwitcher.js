...
const ThemeSwitcher = () => {
    ...
    return (<div className="flex items-center justify-center mx-4">
      {theme === "light" ? (<bi_1.BiMoon className="cursor-pointer" fill="black" size={25} onClick={() => setTheme("dark")}/>) : (<bi_1.BiSun size={25} className="cursor-pointer" onClick={() => setTheme("light")}/>)}
    </div>);
};

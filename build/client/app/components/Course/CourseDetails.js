...
const CheckOutForm_1 = __importDefault(require("../Payment/CheckOutForm"));
...
const CourseDetails = ({ data, stripePromise, clientSecret, setRoute, setOpen: openAuthModal, }) => {
    ...
    return (<div>
        ...
        <div className="w-full flex flex-col-reverse 800px:flex-row">
            ...
            <div>
              {data.benefits?.map((item, index) => (<div className="w-full flex 800px:items-center py-2" key={index}>
                  <div className="w-[15px] mr-1">
                    <io5_1.IoCheckmarkDoneOutline size={20} className="text-black dark:text-white"/>
                  </div>
          ...
          <div className="w-full 800px:w-[35%] relative">
            <div className="sticky top-[100px] left-0 z-50 w-full">
              ...
              <div className="flex items-center">
                {isPurchased ? (<link_1.default className={`${style_1.styles.button} !w-[180px] my-3 font-Poppins cursor-pointer !bg-[crimson]`} href={`/course-access/${data._id}`}>
                    Enter to Course
                  </link_1.default>) : (<div className={`${style_1.styles.button} !w-[180px] my-3 font-Poppins cursor-pointer !bg-[crimson]`} onClick={handleOrder}>
                    Buy Now {data.price}$
                  </div>)}
              </div>
              <br />
      ...
      <>
        {open && (<div className="w-full h-screen bg-[#00000036] fixed top-0 left-0 z-50 flex items-center justify-center">
            <div className="w-[500px] min-h-[500px] bg-white rounded-xl shadow p-3">
              <div className="w-full flex justify-end">
                <io5_1.IoCloseOutline size={40} className="text-black cursor-pointer" onClick={() => setOpen(false)}/>
              </div>
              <div className="w-full">
                {stripePromise && clientSecret && (<react_stripe_js_1.Elements stripe={stripePromise} options={{ clientSecret }}>
                    <CheckOutForm_1.default setOpen={setOpen} data={data} user={user} refetch={refetch}/>

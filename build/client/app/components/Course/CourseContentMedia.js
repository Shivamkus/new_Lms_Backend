...
const CourseContentMedia = ({ data, id, activeVideo, setActiveVideo, user, refetch, }) => {
    ...
    return (<div className="w-[95%] 800px:w-[86%] py-4 m-auto">
      ...
      <div className="w-full p-4 flex items-center justify-between bg-slate-500 bg-opacity-20 backdrop-blur shadow-[bg-slate-700] rounded shadow-inner">
        {["Overview", "Resources", "Q&A", "Reviews", "Test"].map((text, index) => (<h5 key={index} className={`800px:text-[20px] cursor-pointer ${activeBar === index
                ? "text-red-500"
            ...
            <div className="w-full">
              {(course?.reviews && [...course.reviews].reverse())?.map((item, index) => {
                return (<div className="w-full my-5 dark:text-white text-black" key={index}>
                      ...
                      {user.role === "admin" &&
                        item.commentReplies.length === 0 && (<span className={`${style_1.styles.label} !ml-10 cursor-pointer`} onClick={() => {
      ...
      {activeBar === 4 && (<div>
          {data[activeVideo]?.testsQuestions.map((questionItem, index) => (<div className="mb-5 border 1px stroke-orange-50 p-3" key={index}>
                ...
                {questionItem.options.map((optionItem, optionIndex) => (<div className="mb-5" key={optionIndex}>
                      <label className="800px:text-[20px] 800px:inline-block dark:text-white text-black">
                        <input type="radio" name={`question_${index}`} // Unique name for each question
                 value={optionItem.optionText} onChange={(e) => {

...
const FAQ = (props) => {
    ...
    return (<div>
        ...
        <h1 className={`${style_1.styles.title} 800px:text-[40px]`}>
                ...
                <dt className="text-lg">
                  <button className="flex items-start justify-between w-full text-left focus:outline-none" onClick={() => toggleQuestion(q._id)}>
                    ...
                    <span className="ml-6 flex-shrink-0">
                      {activeQuestion === q._id ? (<hi_1.HiMinus className="h-6 w-6 text-black dark:text-white"/>) : (<hi_1.HiPlus className="h-6 w-6 text-black dark:text-white"/>)}
                    </span>
                  </button>
                </dt>
